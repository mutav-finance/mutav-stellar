...
|---|---|---|
| **contract** | A Soroban Rust contract. Contracts live under [`contracts/`](../../contracts/), split by whitepaper stage: the Stage-2 `Fund` vault is in [`contracts/stage2/fund/`](../../contracts/stage2/fund/); the Stage-1 pilot surface is scaffolded under [`contracts/stage1/`](../../contracts/stage1/). | A **rental contract** — the lease agreement between agency and tenant. Database record + CRUD UI in `mutav-app/convex/contracts/`, `mutav-app/src/components/contracts/`, `mutav-app/src/app/(app)/contracts/`. Wholly unrelated to Soroban code. |
