...
|---|---|---|
| **Admin** (cold) | Mutav company multisig / HW wallet on `mutav-app/apps/admin/` | `initialize`, `set_*` (all knobs), allowlist `add` / `remove`, `set_denomination_asset`, propose / execute / cancel `pay_default`, `propose_admin` / `accept_admin`, `set_paused`, `sweep_unknown_token`, `cancel_pending_swap` |
| **Operator** (hot) | KMS-backed Convex Action on `mutav-app` | `record_capital_receipt`, `operator_outbound`, `record_swap_in`, `set_asset_rate`, `publish_snapshot`, `extend_ttl` |
...
- **Operator compromise** can route reserve assets out via the operator outbound entry points (subscription / redemption). The blast-radius constraint is: (i) snapshot publication makes drainage visible in seconds, (ii) admin can `set_paused(true)` to halt outbound, (iii) the rate of drainage is gated by the operator's per-tx Stellar fees + the operator's KMS-side per-action policy, (iv) operator cannot call `pay_default` — that path is admin-only.
...
- **Contract authorization** ([auth docs](https://developers.stellar.org/docs/build/guides/auth/contract-authorization)) — `require_auth` on the admin Address dispatches correctly whether the admin is a regular keypair or, in a future evolution, a contract account implementing `CustomAccountInterface`. The Phase 1 spec assumes regular-keypair admin per the docs' simplicity advice; the migration path to a contract admin is a `propose_admin(contract_addr)` → `accept_admin` from the new contract, with no vault contract change required.
- **CustomAccountInterface signature** ([check-auth tutorials](https://developers.stellar.org/docs/build/guides/auth/check-auth-tutorials)) — `pub fn __check_auth(env: Env, signature_payload: BytesN<32>, signature: BytesN<64>, _auth_context: Vec<Context>)`. Returns `()`, panics on failure. Phase 2 reference for when a `DefaultManager` contract becomes the admin.
...
- **OpenZeppelin Stellar Contracts v0.7.2** ([repo](https://github.com/OpenZeppelin/stellar-contracts)) — explicitly labelled "experimental software" by OZ themselves. Has `audits/` directory but no audit reports cited in README. **Not used as runtime dependency for Phase 1**; we hand-roll the small primitive surface we need (admin/operator auth, paused, two-step rotation) from raw Soroban SDK. Their virtual-decimals-offset formula is referenced for the eventual Stage-2 share-vault refactor.
...
5. **Operator authority is bounded by observable outflow paths**, so a KMS-side policy can attribute every outbound tx to a typed contract entry point.
...
11. **No `CustomAccountInterface` for Phase 1 admin.** Regular keypair per Stellar docs' simplicity advice. This forecloses passkey-based admin for Phase 1 — passkey wallets on Stellar are implemented as contract accounts, which reintroduces the audit-surface complexity this spec sheds. Passkey-based wallets remain a valid choice for **agencies** (recipients of `pay_default`) and for **Phase 2 investor wallets** — those addresses are just destinations from this contract's point of view.
...
fn initialize(
    ...
    admin: Address,
    operator: Address,
    initial_approved_assets: Vec<Address>,    // must contain denomination_asset
...
fn sweep_unknown_token(e: Env, token: Address, amount: i128, destination: Address);  // rejects token ∈ approved_assets
```
...
/// subscription or redemption. Replay-guarded. Writes a PendingSwap entry so the transparency
/// portal can include in-flight value. Emits `outbound(kind, asset, amount, dest, op_tx_hash)`.
...
3. `pay_default` is the **only entry point that decrements the reserve to a non-Mutav-controlled address**. `operator_outbound` decrements but pairs with `record_swap_in` (reserve stays in vault custody via the swap result); `sweep_unknown_token` rejects approved assets; `cancel_pending_swap` does not transfer.
...
**Sweep:**
- `sweep_unknown_token_rejects_approved_asset`
- `sweep_unknown_token_admin_only`
